  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setImagePreview('');
        alert('Action section updated successfully!');
        fetchActionData();
      } else {
        const errorData = await response.json();
        alert(errorData.error || 'Failed to update action section');
      }
    } catch (error) {
      console.error('Error updating action section:', error);
      alert('Failed to update action section. Please try again.');
    } finally {
      setIsSubmitting(false);
    }
  };

  if (loading) return <AdminLayout title="Action Section"><div>Loading...</div></AdminLayout>;

  return (
    <AdminLayout title="Manage Action Section">
      <div style={{ background: '#fff', padding: 24, borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
        <h2 style={{ marginBottom: 24 }}>Action Section Settings</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            {isSubmitting ? 'Updating...' : 'Update Action Section'}
          </button>
        </form>
