...
export async function GET() {
  try {
    ...
    const filteredSections = aboutPage.sections.filter(s => s.name === 'call-to-action');
    return NextResponse.json(filteredSections);
  } catch (error) {
    console.error('Error fetching call to action section:', error);
    return NextResponse.json(
      { error: 'Failed to fetch call to action section' },
      { status: 500 }
    );
  }
}
...
export async function POST(req) {
  try {
    ...
    console.error('Error saving call to action section:', error);
    return NextResponse.json(
      { error: 'Failed to save call to action section' },
      { status: 500 }
    );
  }
} 